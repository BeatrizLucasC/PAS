export default {
  ...
  // 1) Obter todas as perguntas do domínio "Económico" (normalizado)
  getQuestions() {
    const data = Qry_getQuestions.data || [];
    return data.filter((q) => this._normalizeText(q.dominio) === "economico");
  },
  ...
  // 8) Preparar respostas para guardar (domínio normalizado)
  prepareAnswers({ onlyVisible = true } = {}) {
    ...
    // Forçar o domínio a ficar normalizado como 'economico'
  ...
  async onSubmit() {
    try {
      ...
      showAlert(
        "Respostas do domínio económico submetidas com sucesso!",
        "success"
      );
    } catch (e) {
      console.error("Erro em onSubmit (económico):", e);
  ...
  async confirmReplace() {
    try {
      ...
      console.error("Erro em confirmReplace (económico):", e);
  ...
  // 17) Filtro categorias (com widgets económicos)
  categoryOptions() {
  ...
  // Devolve as categorias efetivas de acordo com o Multiselect_CategoriasEco
  effectiveCategoryValues() {
    const selected = Multiselect_CategoriasEco?.selectedOptionValues || [];
  ...
  applyUISubFilters(list) {
    ...
    const status = Select_statusRespostasEco?.selectedOptionValue || "all";
